...
\begin{definition}
  ...
  \begin{example}
    Посмотрим на $MA(\infty)$ со специально подобранными коэффициентами
    \[
     y_t = u_t + ....
    \]
    Попробуем посчитать ковариации
    \[
      \Cov(y_t, y_{t+1}) = 
    \]
    \[
      \Cov(y_t, y_{t+2}) = 
    \]
    Считая дальнейшие ковариации по аналогии, обнаруживаем, что $(y_t)$ — белый шум! 
  Другими словами, один белый шум может являться нетривиальной линейной комбинацией значений другого белого шума. 
  \end{example}
...
\begin{example}
  $MA(1)$ как $MA(1)$ с другими коэффициентами.
...
Обычная корреляция 
\[
\Corr(L, R) = \frac{\Cov(L, R)}{\sqrt{\Var(L) \Var(R)}}
\]
измеряет силу линейной связи между величинами $L$ и $R$.

Частная корреляция
\[
\pCorr(L, R; M_1, M_2, M_3)
\]
измеряет силу линейной связи между величинами $L$ и $R$, если оборвать связи проходящие через 
величины $M_1$, $M_2$, $M_3$. 
Величин, связи через которые элиминируются, не обязательно три. 
Их может быть любое количество. 

\begin{definition}
  Частная корреляция
  \[
    \pCorr(L, R; M_1, M_2, M_3) = \Corr(\tilde L, \tilde R),
  \]
  где $\tilde L$ — величина $L$, «очищенная» от $M_i$-х, а $\tilde R$ — величина $R$, очищенная от $M_i$-x.
  «Очищенные» версии строятся по формулам
  \[
  \tilde L = L - (\alpha_1 M_1 + \alpha_2 M_2 + \alpha_3 M_3), \quad \tilde R = R - (\beta_1 M_1 + \beta_2 M_2 + \beta_3 M_3),
  \]
  так, чтобы у очищенных величин была нулевая ковариация с любой  $M_i$,
  \[
  \Cov(L, M_i) = 0, \quad \Cov(R, M_i) = 0.  
  \]
\end{definition}


\begin{theorem}
  У стационарного процесса $(y_t)$ частная корреляция 
  \[
  \phi_{kk} = \pCorr(y_t, y_{t+k}; y_{t+1}, \ldots, y_{t+k-1})
  \]
  может быть найдена из разложения
  \[
  y_{t} = \alpha + \phi_{k1} y_{t-1} + \phi_{k2} y_{t-2} + \ldots + \phi_{kk} y_{t-k} + w_t, 
  \]
  где ковариации $\Cov(w_t, y_{t-1})$, \ldots, $\Cov(w_t, y_{t-k})$ равны нулю. 
\end{theorem}
...
\[
  ...
  \end{cases}
\]
можно найти все частные корреляции $\phi_{k1}$, \ldots,  $\phi_{kk}$.
Система уравнений называется системой Юла-Волкера (Yule-Walker equations).

Наиболее легко интерпретируется и чаще используется частная корреляция $\phi_{kk}$.
Однако можно проинтерпретировать и другие, например,
\[
\phi_{42} = \pCorr(y_t, y_{t-2}; y_{t-1}, y_{t-3}, y_{t-4}).  
\]

Применив правило Крамера можно получить готовую формулу 
\[
\phi_{kk} = \frac{\det A }{\det B},  
\]
где в матрице $B$ элемент $b_{ij} = \Corr(y_i, y_j)$, а 
матрица $A$ отличается от матрицы $B$ тем, что вместо последнего столбца 
написали $a_{in} = \Corr(y_t, y_{t+i})$.

Если взять ожидание от левой и правой части разложения $y_t$, то можно найти и константу $\alpha$. 

Разложение 
\[
  y_{t} = \alpha + \phi_{k1} y_{t-1} + \phi_{k2} y_{t-2} + \ldots + \phi_{kk} y_{t-k} + w_t, 
\]
из уравнения для поиска частной корреляции можно использовать также и для построения наилучших линейных прогнозов. 
А именно, если мы знаем последние $k$ значений нашего ряда $(y_t)$, то наилучший линейный прогноз 
имеет вид
\[
\hat y_{t+1 \mid t} =   \alpha + \phi_{k1} y_{t} + \phi_{k2} y_{t-1} + \ldots + \phi_{kk} y_{t-k + 1}.
\]
Заметим, что для процесса $(y_t)$ с совместным нормальным распределением наилучшие линейные прогнозы будут просто наилучшими.
...
а случайному процессу в целом, то есть, к последовательности случайных величин. 
  ...
  \begin{definition} \label{de:arp}
    ...
    ряд $(y_t)$ удовлетворяет уравнению
    ...
    y_t - \mu = \beta_1 (y_{t-1} - \mu) + \beta_2 (y_{t-2} - \mu) + \ldots + \beta_p (y_{t-p} - \mu) + u_t
    \]
    можно записать и с помощью оператора лага,
    \[
    P_{AR}(L)\cdot (y_t - \mu) = u_t,  
    \]
    где $P_{AR}(L)$ — многочлен степени $p$ от лага $L$ со свободным членом равным 1, то есть $P_{AR}(L)=1$.
    ...
    \begin{example}
      Рассмотрим уравнение $y_t = 0.5 y_{t-1} + u_t$. 
      Легко найти нестационарное решение. 
      Выбираем любой момент времени, скажем $t=2$, выбираем любое число, например, $100$,
      и дополняем уравнение начальным условием $y_2 = 100$. 

      Все остальные $y_t$ находятся однозначно. 
      В частности $y_3 = 50 + u_3$, $y_4 = 25 + 0.5 u_3 + u_4$, \ldots.
      Можно раскручивать последовательность и в прошлое, переписав рекуррентную формулу в виде $y_{t-1} = 2y_t - 2u_t$.
      Отсюда $y_1 = 200 - 2u_2$, $y_0 = 400 - 4u_2 - 2u_1$, \ldots. 

      Конечно, получающийся процесс нестационарен, $\E(y_2) = 100$, $\E(y_3) = 50$. 

    \end{example}

    \begin{example}
    Рассмотрим уравнение $y_t = 2 y_{t-1} + u_t$ и покажем, что у него \textit{есть} стационарное решение.
  ...
  \begin{definition} \label{de:armapq}
    Ряд $(y_t)$ называется процессом авторегрессии порядка и скользящего среднего порядков $p$ и $q$ (autoregression and moving average of orders $p$ and $q$)
    относительно белого шума $(u_t)$, если выполнено два условия:
    ряд $(y_t)$ удовлетворяет уравнению
    \begin{multline} \label{eq:armapq}
      y_t - \mu = \beta_1 (y_{t-1} - \mu) + \beta_2 (y_{t-2} - \mu) + \ldots + \beta_p (y_{t-p} - \mu) + \\
      + u_t + \alpha_1 u_{t-1} + \ldots \alpha_q u_{t-q},
    \end{multline}
      где последние коэффициенты $\beta_p \neq 0$ и $\alpha_q \neq 0$. 
    ряд $(y_t)$ является процессом $MA(\infty)$ относительно белого шума $(u_t)$:
    \[
      y_t = \mu + u_t + \alpha_1 u_{t-1} + \alpha_2 u_{t-2} + \ldots.
    \]
    Обозначаем такие процессы мы так: $y_t \sim ARMA(p, q)$.
    \end{definition}

    Можно записать уравнение $ARMA(p, q)$ процесса помощью лаговых полиномов
    \[
      P_{AR}(L)\cdot (y_t - \mu) = P_{MA}(L) u_t,
    \]
    где многочлен $P_{AR}(L)$ имеет порядок $p$, многочлен $P_{MA}(L)$ — порядок $q$, 
    и свободные члены в них равны $P_{AR}(0)=1$, $P_{MA}(0)=1$.
